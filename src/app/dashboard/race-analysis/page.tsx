...
export default async function RaceAnalysisPage(props: PageProps) {
  const searchParams = await props.searchParams;
  const season = typeof searchParams.season === 'string' ? searchParams.season : '2026';
  
  const races = await getRaces(season);
  const round = typeof searchParams.round === 'string' ? searchParams.round : (races.length > 0 ? races[0].round.toString() : '1');

  const selectedRace = races.find((r: any) => r.round.toString() === round) || races[0];
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white mb-2">Race Analysis</h1>
          <p className="text-zinc-400">Deep dive into telemetry, lap pace, and pit strategy.</p>
        </div>
        <div className="flex items-center bg-zinc-900 border border-zinc-800 p-2 rounded-lg">
          <RaceSelector races={races} currentRound={round} />
        </div>
      </div>

      {selectedRace && (
        <h2 className="text-xl font-bold text-red-500">{selectedRace.raceName} - Round {round}</h2>
      )}
